...
class AgentAPI {
    ...
    async getMe(authorization, proxy) {
        try {
            return await this.makeRequest('POST', `${this.baseURL}/getMe`, {"referrer_id": 350874250}, authorization, proxy);
        } catch (error) {
        ...
        while (true) {
            for (let no = 0; no < data.length; no++) {
                ...
                try {
                    ...
                    console.log(`===== Tài khoản ${no + 1} | ${firstName} | ip: ${proxyIP} | Update @mitomchanel =====`.green);
